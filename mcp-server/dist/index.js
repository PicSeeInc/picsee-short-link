...
function requireToken() {
    ...
    if (!token)
        throw new Error("Not authenticated. Use the setup_auth tool to store your PicSee API token first.");
    return token;
}
function ok(data) {
    return {
...
// Tools
// ---------------------------------------------------------------------------
server.tool("shorten_url", "Shorten a URL with PicSee. Auto-detects auth mode: if a token is stored, uses authenticated API (api.pics.ee) with advanced features; otherwise falls back to unauthenticated mode (chrome-ext.picsee.tw).", {
...
server.tool("get_analytics", "Get click analytics for a PicSee short link (requires authentication).", {
...
server.tool("list_links", 'List and search PicSee short links (requires authentication). startTime should be the END of the period you want to query backward from, in ISO-like format "YYYY-MM-DDTHH:MM:SS". For example, to see links from March 2026, use "2026-03-31T23:59:59".', {
    startTime: z
        .string()
        .describe('Query backward from this time, format: YYYY-MM-DDTHH:MM:SS. Use the LAST moment of the desired period, e.g. "2026-03-31T23:59:59"'),
    limit: z
        .number()
        .int()
        .min(1)
...
server.tool("edit_link", "Edit an existing PicSee short link (requires Advanced plan authentication). Only include the fields you want to change.", {
    originalEncodeId: z
        .string()
...
server.tool("delete_link", "Delete or recover a PicSee short link (requires authentication).", {
...
server.tool("setup_auth", "Store and verify a PicSee API token. The token is encrypted with AES-256-CBC (machine-specific key) and saved to ~/.openclaw/.picsee_token. The token is verified against the PicSee API before storing.", {
...
server.tool("generate_qr_code", "Generate a QR code URL for a PicSee short link. **Only call this if the user explicitly requests a QR code.** Returns a shortened URL to the QR code image.", {
...
server.tool("generate_analytics_chart", "Generate a line chart URL visualizing daily click statistics. **Only call this if the user explicitly requests a chart or visualization.** Returns a shortened URL to the chart image.", {
    dailyClicks: z
        .array(z.object({
